...
interface AdultVerificationNoticeProps {
  onVerificationStart: () => void;
}

export default function AdultVerificationNotice({
  onVerificationStart,
}: AdultVerificationNoticeProps) {
  return (
    <div className="px-4">
      ...
      <div className="flex flex-col">
        <div className="w-24 h-24 rounded-full border-4 border-[#fa0100] flex items-center justify-center mb-8">
          <span className="text-4xl font-bold">19</span>
        </div>

        <h1 className="text-2xl font-bold mb-4">성인 인증이 필요한 서비스</h1>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 space-y-3">
          <Link href="/login">
            <Button
              variant="outline"
              className="w-full bg-[#26252A] text-white border-0 hover:bg-[#26252A]/80 h-14"
            >
              19세 미만 나가기
            </Button>
          </Link>
